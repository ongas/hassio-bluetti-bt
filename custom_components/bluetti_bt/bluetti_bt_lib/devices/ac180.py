...
class AC180(ProtocolV2Device):
    def __init__(self, address: str, sn: str):
        ...
        # ECO mode DC
        self.struct.add_bool_field('eco_on', 2014)  # ECO DC on/off
        self.struct.add_enum_field('eco_shutdown', 2015, EcoShutdown)  # ECO time mode DC
        
        # ECO mode AC
        self.struct.add_bool_field('eco_on_ac', 2017)  # ECO AC on/off
        self.struct.add_enum_field('eco_shutdown_ac', 2018, EcoShutdown)  # ECO time mode AC
        
        # Charging mode and power lifting
        self.struct.add_enum_field('charging_mode', 2020, ChargingMode)
        self.struct.add_bool_field('silent_charging_on', 2020)  # Alternative bool view for silent charging
        self.struct.add_bool_field('power_lifting_on', 2021)

    @property
    def polling_commands(self) -> List[ReadHoldingRegisters]:
        return super().polling_commands + [
            ReadHoldingRegisters(140, 8),     # Power IO (dc_out, ac_out, dc_in, ac_in)
    ...
    def writable_ranges(self) -> List[range]:
        return [
            range(2011, 2022),  # All control registers from AC/DC switches to power lifting
        ]
